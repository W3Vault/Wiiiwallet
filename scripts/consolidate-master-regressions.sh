...
cp scripts/enforce-wiiicoin-wallet-type.mjs scripts/apply-wiiicoin-consolidation.mjs
node scripts/apply-wiiicoin-consolidation.mjs

python3 <<'PY'
import json
from pathlib import Path

description = 'Wiiiwallet mobile wallet for the Wiiicoin blockchain'
repository = {'type': 'git', 'url': 'https://github.com/W3Vault/Wiiiwallet.git'}

package_path = Path('package.json')
package = json.loads(package_path.read_text())
package['name'] = 'wiiiwallet'
package['description'] = description
package['repository'] = repository
scripts = package.setdefault('scripts', {})
scripts['apply:wiiiwallet-consolidation'] = 'node scripts/apply-wiiicoin-consolidation.mjs'
scripts['audit:wiiiwallet'] = 'node scripts/audit-wiiiwallet-consolidation.mjs'
scripts['patches'] = 'npx patch-package && npm run audit:wiiiwallet'
package_path.write_text(json.dumps(package, indent=2) + '\n')

lock_path = Path('package-lock.json')
if lock_path.exists():
    lock = json.loads(lock_path.read_text())
    lock['name'] = 'wiiiwallet'
    root = lock.get('packages', {}).get('')
    if isinstance(root, dict):
        root['name'] = 'wiiiwallet'
        root['description'] = description
    lock_path.write_text(json.dumps(lock, indent=2) + '\n')

fastfile_path = Path('fastlane/Fastfile')
fastfile = fastfile_path.read_text()
fastfile = fastfile.replace(
...
cat > scripts/enforce-wiiicoin-wallet-type.mjs <<'EOF'
// Consolidated source is tracked directly. Postinstall validates it so a
// future merge cannot silently replace Wiiiwallet or namespace changes.
import './audit-wiiiwallet-consolidation.mjs';
EOF
