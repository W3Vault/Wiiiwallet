...
type WiiicoinLogoProps = SvgProps & {
  color?: string;
};

/** Vector rendering of the supplied Wiii_Logo_4.svg artwork. */
const WiiicoinLogo: React.FC<WiiicoinLogoProps> = ({ color, opacity, ...props }) => {
  const fillColor = color ?? (typeof opacity === 'number' && opacity < 1 ? '#FFFFFF' : '#A654A0');
  ...
  return (
    <Svg
      ...
      accessibilityLabel="Wiiicoin logo"
      opacity={opacity}
      {...props}
    >
      <Path
